...
void ofApp::reloadProject(string path) {
    projectPath = path;
    ofDirectory dir(path); if(!dir.exists()) dir.create(true);
    
    string configsDir = ofFilePath::join(path, "configs");
    ofDirectory dConf(configsDir); if(!dConf.exists()) dConf.create();
    
    string mediaDir = ofFilePath::join(path, "media");
    ofDirectory dMedia(mediaDir); if(!dMedia.exists()) dMedia.create();
...
void ofApp::update(){
    ...
    while((size = net.receive(packetBuffer, 65535)) > 0) {
        ...
        if(h->id != PACKET_ID) continue;
        
        if(h->type == PKT_HEARTBEAT) {
            HeartbeatPacket* p = (HeartbeatPacket*)packetBuffer;
            if(string(p->peerId) == identity.myId) continue; // Ignore own echo
            ...
            string fullPath = ofFilePath::join(content.mediaPath, name);
            ...
            string finalPath = ofFilePath::join(content.mediaPath, incoming.name);
            string tmpPath = finalPath + ".tmp";
            ofBufferToFile(tmpPath, incoming.buf);
            ofFile(tmpPath).renameTo(finalPath, true, true);
            if(incoming.name == "content.mp4") content.setup(content.mediaPath);
        }
    }
}

void ofApp::draw(){
    warper.draw(content.getTexture());
...
void ofApp::drawUI() {
    ...
    if(ImGui::Begin("invasiv", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
        ...
        if(ImGui::TreeNode("Surfaces")) {
            ...
            if(ImGui::Button("+")) warper.addLayer(warper.targetPeerId, &net);
            ImGui::SameLine();
            if(ImGui::Button("-")) warper.removeLayer(warper.targetPeerId, &net);
            ImGui::TreePop();
        }
        ...
        if(warper.selectedIndex < (int)subset.size()) {
            ...
            if(ImGui::Selectable("no edit", warper.editMode == EDIT_NONE)) warper.editMode = EDIT_NONE;
            if(ImGui::Selectable("edit texture", warper.editMode == EDIT_TEXTURE)) warper.editMode = EDIT_TEXTURE;
            if(ImGui::Selectable("edit mapping", warper.editMode == EDIT_MAPPING)) warper.editMode = EDIT_MAPPING;
            
            ImGui::ColorButton("##c1", ImVec4(0.8, 0.2, 0.3, 1)); ImGui::SameLine();
            ImGui::ColorButton("##c2", ImVec4(0.2, 0.3, 0.4, 1));
        }
        
        ImGui::Separator();
        if(ImGui::Button("Sync Content")) net.offerFile("content.mp4");
    }
    ImGui::End();
    gui.end();
}
...
void ofApp::mousePressed(int x, int y, int button){
    if(!net.isMaster || ImGui::GetIO().WantCaptureMouse) return;
...
void ofApp::mouseReleased(int x, int y, int button){
    if(net.isMaster) warper.mouseReleased(net);
}

void ofApp::keyPressed(int key){
    if(key == 'm') net.setRole(true);
    if(key == 'p') net.setRole(false);
    if(key == 't') content.toggle();
}