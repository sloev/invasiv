...
public:
    ...
    void setMediaPath(string _path) {
        lock(); mediaPath = _path; unlock();
    }
    
    void setRole(bool master) {
        lock();
        isMaster = master;
        if(isMaster) ofLogNotice() << "Switched to MASTER";
    ...
    bool hasActiveMaster() {
        for(auto& p : peers) {
            if(p.second.isMaster) return true;
        }
        return false;
    }
    ...
    void sendStructure(string jsonStr) {
        if(!isMaster) return;
        string p = "  "; p[0] = PACKET_ID; p[1] = PKT_STRUCT;
        p += jsonStr;
    ...
    void offerFile(string filename) {
        if(!isMaster) return;
        lock(); pendingFiles.push(filename); unlock();
    }
    ...
    void updatePeers() {
        ...
        for(auto it = peers.begin(); it != peers.end(); ) {
            if(now - it->second.lastSeen > 5.0) it = peers.erase(it);
            else ++it;
        }
    }
...
private:
    ...
    void threadedFunction() {
        while(isThreadRunning()) {
            // Heartbeat
            if(ofGetFrameNum() % 60 == 0) sendHeartbeat();
            
            // File Sync
            string filename = "";
            lock();
            if(!pendingFiles.empty()) { filename = pendingFiles.front(); pendingFiles.pop(); }
            unlock();
            
            if(filename != "") transferFile(filename);
    ...
    void transferFile(string filename) {
        ...
        PacketHeader end; end.type = PKT_FILE_END;
