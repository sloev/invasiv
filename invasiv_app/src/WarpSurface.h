...
public:
    ...
    // Display Mesh (High Res)
    ofVboMesh renderMesh;
    ofMesh sourceMesh;

    // Control Handles (Low Res)
    vector<glm::vec3> controlRender;
    vector<glm::vec3> controlSource;

    int rows = 3;
    int cols = 3;
    int resolution = 20; // Subdivisions per grid cell
    ...
    // [MODIFIED] Only changes resolution, DOES NOT reset controls
    void setResolution(int res)
    {
        ...
        {
            resolution = res;
            // This only rebuilds the high-res VBO,
            // it keeps your controlRender/controlSource points intact.
            rebuildMeshTopology();
        }
    }

    // [NEW] Smart Resizing: Interpolates old shape into new grid
    void setGridSize(int newRows, int newCols)
    {
        ...
        // 2. Resample existing shape into new vectors
        for (int y = 0; y <= newRows; y++)
        {
            for (int x = 0; x <= newCols; x++)
            {
                ...
                // Ask: "Where is UV(u,v) on the OLD surface?"
        ...
        // 4. Rebuild VBO with new topology
        selectedPoint = -1; // Deselect to avoid crash
        rebuildMeshTopology();
    }
    ...
    // [NEW] Helper to sample a specific point on the Catmull-Rom surface
    // This allows us to "Bake" the current curve into the new grid
    glm::vec3 getSurfacePoint(const vector<glm::vec3> &ctrls, int r, int c, float u, float v)
    {
        ...
        // Handle Edge Case (u=1.0)
        if (xInt >= c)
            xInt = c - 1;
        if (yInt >= r)
            yInt = r - 1;

        float tX = xVal - xInt; // Local interpolation factor
        float tY = yVal - yInt;

        // 1. Interpolate 4 horizontal rows to get 4 points aligned vertically
        glm::vec3 pts[4];
        for (int k = 0; k < 4; k++)
        {
            int rowY = yInt + (k - 1);
        ...
        // 2. Interpolate those 4 points vertically
    ...
    {
        if (meshDirty && (ofGetElapsedTimef() - lastMeshUpdate > updateInterval))
        {
            updateMeshPositions();
        }
        ...
        if (renderMesh.getTexCoords().capacity() < srcVerts.size())
            renderMesh.getTexCoords().reserve(srcVerts.size());

        for (const auto &v : srcVerts)
        {
            renderMesh.addTexCoord(tex.getCoordFromPercent(v.x, v.y));
        }
    ...
    void drawDebug(float w, float h, int mode)
    {
        ...
        for (int y = 0; y <= rows; y++)
        {
            ofPolyline line;
            for (int x = 0; x <= cols; x++)
                line.addVertex(verts[x + y * (cols + 1)]);
            line.draw();
        }
        for (int x = 0; x <= cols; x++)
        {
    ...
    // Accessors
    void setContentId(string id) { contentId = id; }
    ...
    ofJson toJson()
    {
        ...
        j["id"] = id;
        j["owner"] = ownerId;
        for (auto &v : controlRender)
            j["geo"].push_back({{"x", v.x}, {"y", v.y}});
        for (auto &v : controlSource)
            j["tex"].push_back({{"x", v.x}, {"y", v.y}});
        return j;
    }
